...
class METERTransformerSS(pl.LightningModule):
    def __init__(self, config):
        ...
        # 추가 
        elif 'kobert' in config['tokenizer']:
        ...
        if torch.distributed.is_initialized():
            if torch.distributed.get_rank() == 0:
                ...
                if 'klue' in config['tokenizer']:
                    AutoModel.from_pretrained("klue/roberta-base")
                elif 'roberta' in config['tokenizer']:
                    RobertaModel.from_pretrained(config['tokenizer'])
                elif 'kobert' in config['tokenizer']: # 추가 
        ...
        if 'klue' in config['tokenizer']:
            self.text_transformer = AutoModel.from_pretrained('klue/roberta-base')
            print('################## Apply KlueBert! ##################')
        elif 'roberta' in config['tokenizer']:
            self.text_transformer = RobertaModel.from_pretrained(config['tokenizer'])
        elif 'kobert' in config['tokenizer']: # 추가 
            self.text_transformer = BertModel.from_pretrained("monologg/kobert")
            print('################## Apply KoBERT! ##################')
        else:
